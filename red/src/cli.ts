...
import { n8nWorkflow } from "./workflow.ts";
...
export const usage =
  ...
  "  create    provision and verify the n8n server and its storage tier\n" +
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(n8nWorkflow, withFile);
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(n8nWorkflow, defaultArgs(args));
  }

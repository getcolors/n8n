...
async def test_retired_delete_ignores_stale_host(monkeypatch,tmp_path,event,retired,expected):
    ...
    result=await tools.ansible_step({'profile':'test','workdir':str(tmp_path),'blue/event':event,'n8n/already-destroyed':retired,'ip':'203.0.113.19','ssh-private-key-path':str(tmp_path/'removed-key')})

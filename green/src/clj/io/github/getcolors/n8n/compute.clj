...
(defn requirements [opts]
 (let [ssh (source-cidrs opts "ssh-sources" "n8n-ssh-sources") http (source-cidrs opts "http-sources" "n8n-http-sources") http (if (= ["cloudflare"] http) (vec (json/parse-string (slurp (io/resource "io/github/getcolors/n8n/origin-ranges.json")))) http)]
...
(defn infrastructure-step [opts]
 (let [planning? (or (= :build (:green/event opts)) (:green/dry-run opts))
       result (if planning? (planning/plan-deployment (settings opts) topology (requirements opts)) (orchestration/orchestrate (settings opts) topology (requirements opts)))]
  (when planning?
  ...
  (attach opts result)))
(defn load-step [opts] (if (:green/dry-run opts) opts (attach opts (inspection/read-deployment (settings opts) (into {} (System/getenv)) {} (requirements opts)))))

...
for f in n8n.dump n8n-data.tar.gz manifest.txt; do
  rclone copyto "backup:$BACKUP_BUCKET/<{ profile }>/$STAMP/$f" "$WORK/$f" 2>/dev/null
done
...
psql_admin "DROP DATABASE IF EXISTS $SCRATCH_DB WITH (FORCE)" >/dev/null 2>&1 || true
psql_admin "CREATE DATABASE $SCRATCH_DB OWNER <{ neon-role }>" >/dev/null
...
for _ in $(seq 1 60); do
  ...
  if docker compose exec -T n8n-rehearsal sh -lc \
      "wget -q -O /dev/null http://127.0.0.1:<{ n8n-port }>/healthz/readiness" >/dev/null 2>&1; then
    ready=1; break
  fi
  sleep 2
done

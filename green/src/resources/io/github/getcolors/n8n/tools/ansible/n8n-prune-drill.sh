...
for _ in $(seq 1 60); do
  docker compose exec -T n8n-prune sh -lc \
    "wget -q -O /dev/null http://127.0.0.1:<{ n8n-port }>/healthz/readiness" >/dev/null 2>&1 && { ready=1; break; }
  sleep 2
done
...
docker compose exec -T \
  ...
  n8n-prune sh -lc '
    wget -q -O /dev/null --post-data="{\"email\":\"drill@example.invalid\",\"firstName\":\"D\",\"lastName\":\"R\",\"password\":\"Aa1drillpassword\"}" \
      --header="Content-Type: application/json" http://127.0.0.1:'"<{ n8n-port }>"'/rest/owner/setup 2>/dev/null || true

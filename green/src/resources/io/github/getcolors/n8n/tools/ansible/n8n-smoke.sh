...
n8n_api() { docker compose exec -T n8n wget -q -O - "http://127.0.0.1:<{ n8n-port }>$1" 2>/dev/null; }
...
case ",$ten," in
  *",<{ neon-tenant-id }>,"*) pass "A1 tenant <{ neon-tenant-id }> attached" ;;
  *) fail "A1 tenant <{ neon-tenant-id }> not attached (saw: ${ten:-none})" ;;
esac
...
if curl -sf --resolve "<{ n8n-host }>:443:127.0.0.1" "https://<{ n8n-host }>/healthz" -o /dev/null 2>/dev/null; then
  pass "B2 origin serves a valid certificate for <{ n8n-host }>"
else
  fail "B2 origin TLS failed for <{ n8n-host }> (ACME may not have completed)"
fi
...
[ "$wh" = "https://<{ n8n-host }>/" ] \
  && pass "B5 N8N_WEBHOOK_URL is https://<{ n8n-host }>/" \

...
(defn stamp-green [s sha]
  (when (re-find #"\(def \^:private neon-sha (?:nil|\"[0-9a-f]{40}\")\)" s)
    (str/replace-first s #"\(def \^:private neon-sha (?:nil|\"[0-9a-f]{40}\")\)"
                       (str "(def ^:private neon-sha \"" sha "\")"))))

(defn stamp-red [s sha]
  (let [pinned (str "\"package-neon-red\": \"github:getcolors/neon#" sha "\",")]
    (cond (str/includes? s "\"package-neon-red\": null,")
          (str/replace-first s "\"package-neon-red\": null," pinned)
          (re-find #"\"package-neon-red\": \"github:getcolors/neon#[0-9a-f]{40}\"," s)
          (str/replace-first s #"\"package-neon-red\": \"github:getcolors/neon#[0-9a-f]{40}\"," pinned))))
...
(defn blue-pinned-meta [sha]
  (str "# dependencies = [\"package-neon-blue\", \"blue\"]\n"
       "#\n"
       "# [tool.uv.sources]\n"
       "# package-neon-blue = { git = \"https://github.com/getcolors/n8n.git\", rev = \"" sha "\", subdirectory = \"blue\" }\n"
...
(defn stamp-blue [s sha]
  ...
  (cond (str/includes? s blue-unpinned-meta)
        (-> s
            ...
            (str/replace-first #"(?s)# UNPINNED:.*?NEON_LIB_ROOT=/path/to/neon\n"
                               "# Stamped by `bb pin`. NEON_LIB_ROOT=/path/to/neon still overrides the\n# pin with a working tree.\n"))
        (re-find #"neon\.git\", rev = \"[0-9a-f]{40}\"" s)
        (str/replace-first s #"neon\.git\", rev = \"[0-9a-f]{40}\""
                           (str "neon.git\", rev = \"" sha "\""))))

(def sites
  [{:path "../skills/package-neon-green/green" :stamp stamp-green}
   {:path "../skills/package-neon-red/red" :stamp stamp-red}
   {:path "../skills/package-neon-blue/blue" :stamp stamp-blue}])

(let [dirty (git "status" "--porcelain") sha (git "rev-parse" "HEAD") remotes (git "branch" "-r" "--contains" sha)]
  (cond (seq dirty) (do (binding [*out* *err*] (println "neon working tree is dirty; commit before pinning")) (System/exit 2))
        (not (str/includes? (str remotes) "origin/")) (do (binding [*out* *err*] (println "neon HEAD is not pushed")) (System/exit 2))

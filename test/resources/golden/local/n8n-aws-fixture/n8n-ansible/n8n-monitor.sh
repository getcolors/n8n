...
[ "${disk:-0}" -lt "80" ] || note "disk ${disk}% over threshold"

mem=$(free | awk '/^Mem:/ {printf "%d", $3*100/$2}')
[ "${mem:-0}" -lt "85" ] || note "memory ${mem}% over threshold"
...
docker compose exec -T n8n wget -q -O /dev/null "http://127.0.0.1:5678/healthz" \
...
if [ "$fail" -eq 0 ]; then
  ...
  backup_put_string "ok $(date -u +%Y%m%dT%H%M%SZ)" \
    "$BACKUP_BUCKET/n8n-aws-fixture/heartbeat" 2>/dev/null \

...
role="n8n"
db="n8n"
bucket="n8n-aws-fixture-neon-123456789012-us-east-1"
prefix="n8n-aws-fixture/data"
...
export RCLONE_CONFIG_R2_REGION="us-east-1"
...
if [ "$(rclone cat "r2:$bucket/$prefix/.colors-ready" 2>/dev/null)" != "n8n-aws-fixture" ]; then
  ready=$(mktemp); printf '%s' "n8n-aws-fixture" > "$ready"
  rclone copyto "$ready" "r2:$bucket/$prefix/.colors-ready"
  rm -f "$ready"
  back=$(rclone cat "r2:$bucket/$prefix/.colors-ready" 2>/dev/null || true)
  if [ "$back" != "n8n-aws-fixture" ]; then
    echo "neon-smoke: ready marker read back as '$back', expected 'n8n-aws-fixture'" >&2
    exit 1
  fi

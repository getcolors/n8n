...
if [ "$(rclone cat "r2:$bucket/$prefix/.colors-ready" 2>/dev/null)" != "n8n-fixture" ]; then
  ready=$(mktemp); printf '%s' "n8n-fixture" > "$ready"
  rclone copyto "$ready" "r2:$bucket/$prefix/.colors-ready"
  rm -f "$ready"
  back=$(rclone cat "r2:$bucket/$prefix/.colors-ready" 2>/dev/null || true)
  if [ "$back" != "n8n-fixture" ]; then
    echo "neon-smoke: ready marker read back as '$back', expected 'n8n-fixture'" >&2
    exit 1
  fi

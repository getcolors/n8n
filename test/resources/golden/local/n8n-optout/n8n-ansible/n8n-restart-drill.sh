...
if [ "$MODE" = "reboot-check" ]; then
  ...
  if [ -n "$wf" ]; then
    st=$(docker compose exec -T \
      ...
      -e N8N_PORT="5678" n8n node /opt/n8n-scripts/acceptance.js run-code "$wf" 2>/dev/null | tr -d '\r' | tail -1)
    ...
    docker compose exec -T \
      -e OWNER_EMAIL="operator@example.com" -e OWNER_PW="$(cat /etc/n8n/secrets/owner-password)" \
      -e N8N_PORT="5678" n8n node /opt/n8n-scripts/acceptance.js delete "$wf" >/dev/null 2>&1 || true
  fi
  rm -f "$WITNESS_FILE"
...
docker compose exec -T \
  -e OWNER_EMAIL="operator@example.com" -e OWNER_PW="$(cat /etc/n8n/secrets/owner-password)" \
  -e N8N_PORT="5678" n8n node /opt/n8n-scripts/acceptance.js create "$WIT" >/dev/null 2>&1

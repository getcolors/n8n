...
for variant in colors optout; do
  ...
  # n8n's own operator secret, in this package's play.
  if ! grep -q "lookup('env','COLORS_PAR_N8N_ENCRYPTION_KEY')" "$actual/n8n-ansible/n8n.yml"; then
    echo "golden: $profile no longer renders COLORS_PAR_N8N_ENCRYPTION_KEY as a lookup" >&2; exit 1
  fi
